...
extern "C"
{
    ...
    // --- float32 variant (for zero-copy DLpack float tensors) ---

    /// Kernel: L2 norm reduction for float32 input (accumulate sum-of-squares).
    __global__ void l2_norm_kernel_f32(
        const float *__restrict__ input,
        size_t n,
        float *__restrict__ out_sum)
    {
        const size_t idx = (size_t)blockIdx.x * (size_t)blockDim.x * 2 + (size_t)threadIdx.x;

        float local_sum = 0.0f;
        if (idx < n)
        {
            const float v = __ldg(input + idx);
            local_sum += v * v;
        }
        const size_t idx2 = idx + blockDim.x;
        if (idx2 < n)
        {
            const float v = __ldg(input + idx2);
            local_sum += v * v;
        }

        // Reduce within block using shared reduction helper (double path exists);
        // for float, promote to double for reduction stability then cast back.
        const double block_sum = block_reduce_sum((double)local_sum);
        if (threadIdx.x == 0)
        {
            atomicAdd(out_sum, (float)block_sum);
        }
    }
    ...
    /// Launch L2 norm reduction for a single float32 vector.
    /// Writes the inverse norm (1 / ||x||) into `inv_norm_out_d`.
        ...
        cudaStream_t stream)
    {
        ...
        const int blockSize = 256;
        const size_t elements_per_block = (size_t)blockSize * 2;
        size_t gridSize = (input_len + elements_per_block - 1) / elements_per_block;
        gridSize = (gridSize == 0) ? 1 : gridSize;
        const size_t maxBlocks = 4096;
        if (gridSize > maxBlocks)
